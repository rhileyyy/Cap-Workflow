...
export function buildProductPrompt(s) {
  ...
  const sideLogoLine = sideLogos.length > 0
    ? s.stripeCount > 0
      ? `Sewn embroidered logo from the side reference image placed directly over the stripes on the lower mesh of the ${sideLogos.join(' and ')}. The logo sits on top of the stripes, not beside them. Black outline embroidery, reproduced exactly from the reference.`
      : `Sewn embroidered logo from the side reference image on the lower mesh of the ${sideLogos.join(' and ')}, centred vertically on the side panel. Black outline embroidery, reproduced exactly from the reference.`
    : '';
