...
export function buildProductPrompt(s) {
  ...
  const sideLogoLine = sideLogos.length > 0
    ? `Sewn embroidered logo from the side reference image on the ${sideLogos.join(' and ')}, positioned on the lower mesh over the stripes. Reproduce exactly from the reference with  outline embroidery.`
    : '';
...
export function buildModelPrompt(modelKey, s) {
  ...
  const logoRule = 'The front logo is embroidered exactly from the reference image — same shapes, text, proportions. Do NOT redraw or substitute it. complementary colored outline embroidery, 3D raised with visible thread detail.';
