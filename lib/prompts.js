...
export function buildProductPrompt(s) {
  ...
  const stripeLine = s.stripeCount === 0
    ...
    : `${s.stripeCount} sewn ${describeColor(s.stripeColor)} stripe${s.stripeCount > 1 ? 's' : ''} on BOTH side mesh panels, positioned in the BOTTOM THIRD of the mesh, close to the brim edge — NOT in the middle or upper portion of the mesh. Stripes run horizontally parallel to the brim, tightly grouped with only 3-4mm between stripes.`;
  ...
  const sideLogoLine = sideLogos.length > 0
    ? s.stripeCount > 0
      ? `Images 3 and 4 are both the SIDE PANEL DESIGN. Reproduce it EXACTLY on the ${sideLogos.join(' and ')} — every shape, letter, colour, and detail must match the reference precisely, including any white or light-coloured elements within the design which must be reproduced as-is and NOT filled in or simplified. The side design is embroidered directly ON TOP OF the stripes in the bottom third of the mesh — the design is the foreground, stripes are the background behind it. Do NOT redraw or substitute any element. Raised 3D embroidery with visible stitches.`
      : `Images 3 and 4 are both the SIDE PANEL DESIGN. Reproduce it EXACTLY on the ${sideLogos.join(' and ')} in the bottom third of the mesh panel — every shape, letter, colour, and detail must match the reference precisely, including any white or light-coloured elements within the design which must be reproduced as-is and NOT filled in or simplified. Do NOT redraw or substitute any element. Raised 3D embroidery with visible stitches.`
    : '';
