...
export function buildProductPrompt(s) {
  ...
  const sideLogoLine = sideLogos.length > 0
    ? s.stripeCount > 0
      ? `Images 3 and 4 are both the SIDE PANEL DESIGN (same design shown twice for emphasis). Reproduce it EXACTLY on the ${sideLogos.join(' and ')} — every shape, letter, colour, and detail must match precisely. Place it directly over the stripes on the lower mesh, sitting on top of the stripes. Do NOT redraw, simplify, or substitute any element. Render as raised 3D embroidery with black outline, visible stitches.`
      : `Images 3 and 4 are both the SIDE PANEL DESIGN (same design shown twice for emphasis). Reproduce it EXACTLY on the ${sideLogos.join(' and ')} — every shape, letter, colour, and detail must match precisely. Centre it on the lower mesh panel. Do NOT redraw, simplify, or substitute any element. Render as raised 3D embroidery with black outline, visible stitches.`
    : '';
