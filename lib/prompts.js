...
const PROMPT = {
  ...
  // ── Logo lockdown ─────────────────────────────────────────────────────────
  logoLockdown: 'Images 2 and 3 are both the front logo — same design shown twice. Embroider this design on the centre of the front panel EXACTLY as shown — same shapes, same text, same proportions, same colours. Do NOT redraw, simplify, or substitute any part of it.',
...
export function buildProductPrompt(s) {
  ...
  const colourLine = `Change the cap colours: make the front panel ${front}, the mesh ${mesh}, the brim ${brim}, and the snapback ${snapback}.`
    + (s.sandwichBrim
  ...
  const sideLogoLine = sideLogos.length > 0
    ? s.stripeCount > 0
      ? `Images 4 and 5 are both the SIDE PANEL DESIGN. Reproduce it EXACTLY on the ${sideLogos.join(' and ')} — every shape, letter, colour, and detail must match the reference precisely, including any white or light-coloured elements which must NOT be filled in or simplified. The side design sits ON TOP OF the stripes, foreground over background. Raised 3D embroidery with visible stitches.`
      : `Images 4 and 5 are both the SIDE PANEL DESIGN. Reproduce it EXACTLY on the ${sideLogos.join(' and ')} in the lower mesh area — every shape, letter, colour, and detail must match precisely, including any white or light-coloured elements which must NOT be filled in or simplified. Raised 3D embroidery with visible stitches.`
    : '';
