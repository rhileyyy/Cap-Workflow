...
const PROMPT = {
  ...
  construction: 'High crown structured cotton front panel — solid square face, single piece of fabric, no visible centre seam. Mesh rear panels with clearly visible woven honeycomb texture. Clean sharp seam where solid front meets mesh sides. Pre-curved brim, smooth clean edge with absolutely no stitching, no topstitching, no stitch lines visible on the brim surface at all. Squatchee button on top crown. Snapback closure at rear.',
...
export function buildProductPrompt(s) {
  ...
  const sideLogoLine = sideLogos.length > 0
    ? `Sewn 3D embroidered logo from the side reference image on the ${sideLogos.join(' and ')}, positioned on the lower mesh over the stripes. Reproduce exactly from the reference with  outline embroidery.`
    : '';
...
export function buildModelPrompt(modelKey, s) {
  ...
  const logoRule = 'The front logo is embroidered in 3D exactly from the reference image — same shapes, text, proportions. Do NOT redraw or substitute it. complementary colored outline embroidery, 3D raised with visible thread detail.';
